...
from keyboards import main_menu_by_role  # Нова функція
...
async def switch_role_menu(message: Message):
    async with async_session() as session:
        ...
        current = active_role.get(message.from_user.id, user.primary_role)
        await message.answer(
            f"📋 **Ваші ролі:** {', '.join(roles)}\n"
            f"👉 **Поточна активна роль:** {current.capitalize()}\n\n"
            f"Оберіть роль для роботи:",
            reply_markup=kb,
            parse_mode="Markdown"
        )
...
async def set_active_role(callback: CallbackQuery):
    ...
    # Змінюємо активну роль у словнику
    ...
    async with async_session() as session:
        ...
        # ОНОВЛЮЄМО МЕНЮ - використовуємо нову функцію main_menu_by_role
        await callback.message.answer(
            f"✅ **Активну роль змінено на:** *{new_role.capitalize()}*\n\n"
            f"Ваше меню оновлено. Ось доступні функції:",
            reply_markup=main_menu_by_role(new_role),  # ЗМІНА ТУТ
            parse_mode="Markdown"
        )
