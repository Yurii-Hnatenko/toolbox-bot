...
async def skip_photo(callback: CallbackQuery, state: FSMContext):
    ...
    if not results:
        await callback.message.answer("❌ Немає даних про перевірку.")
        await state.clear()
        return
    ...
    try:
        async with async_session() as session:
            ...
            box_status = box_result.scalar_one_or_none()
            if not box_status:
        ...
        result_text = f"📋 **Результати перевірки**\n"
        result_text += "━" * 25 + "\n\n"
        result_text += f"📦 **Ящик:** {toolbox.name}\n"
...
async def save_photo_and_check(message: Message, state: FSMContext):
    ...
    if not results:
        await message.answer("❌ Немає даних про перевірку.")
        await state.clear()
        return
